    ...
    public ResponseEntity<String> execute(String user, String pass) {
    	...
    	UserManager userManager = new UserManagerImpl();	
   		...
   		if (!userManager.doesUserAlreadyExist(user)) {
    		...
    		return new ResponseEntity<String>("role", HttpStatus.BAD_REQUEST);//new WebApplicationException(Response.status(Response.Status.BAD_REQUEST).entity("user already exists").type(MediaType.TEXT_PLAIN).build());		
    	}
    }
}
